...
func (metainfo metainfo_impl) Raw() bencode.Dictionary {
	return bencode.Dictionary(metainfo)
}

func (metainfo metainfo_impl) InfoHash() (hash.Hash, error) {
	encodeInfo, err := bencode.Encode(metainfo["info"])
	if err != nil {
		return hash.Hash{}, err
	}

	infohash := hash.Calculate([]byte(encodeInfo))
